...
Engine::Engine(int screenWidth, int screenHeight, const std::string &title)
{
    ...
    inform({"Game : C++",
            toStr("SDL  : ", std::to_string(SDL_MAJOR_VERSION), ".", std::to_string(SDL_MINOR_VERSION)),
            m_title,
            toStr("Window ", std::to_string(m_screenWidth), " x ", std::to_string(m_screenHeight))},
           true);
...
void Engine::gameLoop()
{
    do {
        ...
        if (m_gamePerformance.ticks % static_cast<long>(15 * 1000) == 0) {
            inform(toStr("Ticks: ", m_gamePerformance.ticks, " FPS: ", m_gamePerformance.framesPerSecond,
                         " Delay: ", delay, " Delta time: ", m_gamePerformance.deltaTime, " seconds"));
        }
    }
    while (!gameStateEqual(State::Game::stopped));
...
auto Engine::loadTexture(const std::string &id, int rows, int  columns, const std::string &filename) -> bool
{
    ...
    inform(toStr("Added Texture: ", id, ", Rows [", rows, "], Columns [", columns,
